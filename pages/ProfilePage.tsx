...
const ProfilePage = () => {
    ...
    const memberSince = user?.created_at ? new Date(user.created_at).toLocaleDateString('es-PE', { month: 'long', year: 'numeric' }) : 'Fecha desconocida';
            ...
            <header className="sticky top-0 z-50 flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#233c48] bg-background-dark px-4 lg:px-10 py-3 shadow-md">
                <div className="flex items-center gap-4 lg:gap-8 cursor-pointer" onClick={() => navigate('/')}>
                    ...
                    <nav className="hidden lg:flex items-center gap-6">
                        <Link to="/mercados" className="text-text-secondary hover:text-white text-sm font-medium transition-colors">Mercados</Link>
                ...
                <div className="flex flex-1 justify-end gap-4 lg:gap-8">
                    <button onClick={handleSignOut} className="text-[#92b7c9] hover:text-white text-sm font-medium transition-colors">Cerrar Sesión</button>
                    <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10 ring-2 ring-primary/20" style={{ backgroundImage: `url("${avatarUrl}")` }}></div>
                </div>
            </header>
                        ...
                        <aside className="lg:col-span-3 lg:sticky lg:top-24 h-fit space-y-6">
                            <div className="bg-surface-dark rounded-xl p-6 border border-[#233c48] flex flex-col items-center text-center relative overflow-hidden group">
                                ...
                                <div className="relative">
                                    <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full h-32 w-32 border-4 border-surface-dark shadow-xl" style={{ backgroundImage: `url("${avatarUrl}")` }}></div>
                                ...
                                <div className="mt-4 flex flex-col gap-1">
                                    ...
                                    <p className="text-text-secondary text-sm">Miembro desde {memberSince}</p>
                                </div>
                        ...
                        <div className="lg:col-span-9 flex flex-col gap-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="bg-surface-dark border border-[#233c48] p-5 rounded-xl flex flex-col justify-between relative overflow-hidden">
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <p className="text-text-secondary font-medium text-sm">Apuestas Gratuitas</p>
                                            <div className="flex items-baseline gap-2 mt-1">
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-[#1a2932] border-b border-[#233c48]">
                                                ...
                                                <th className="p-4 text-xs font-semibold text-text-secondary uppercase tracking-wider text-right">Estado</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-[#233c48]">
                                            <tr className="hover:bg-[#1e2f39] transition-colors group cursor-pointer">
                                                ...
                                                <td className="p-4 text-right text-yellow-400 text-sm font-medium">Pendiente</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
